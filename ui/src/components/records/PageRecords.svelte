<script>
    ...
    $: $pageTitle = $activeCollection?.name || "数据集";
...
{:else if !$collections.length}
    <PageWrapper center>
        <div class="placeholder-section m-b-base">
            ...
            {#if $hideControls}
                <h1 class="m-b-10">还没有任何数据集</h1>
            {:else}
                <h1 class="m-b-10">创建您的第一个数据集！</h1>
                <button
                    type="button"
    ...
    <PageWrapper class="flex-content">
        <header class="page-header">
            <nav class="breadcrumbs">
                <div class="breadcrumb-item">数据集</div>
            ...
            <div class="inline-flex gap-5">
                {#if !$hideControls}
                    <button
                        ...
                        use:tooltip={{ text: "编辑数据集", position: "right" }}
