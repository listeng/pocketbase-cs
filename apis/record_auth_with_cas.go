...
func validateCASTicket(c *core.Collection, e *core.RequestEvent) (string, string, error) {
	...
	e.App.Logger().Info("cas login", slog.String("user", username))
...
func recordAuthWithCAS(e *core.RequestEvent) error {
	...
	username, userType, err := validateCASTicket(collection, e)
	if err != nil {
		return err
	}
